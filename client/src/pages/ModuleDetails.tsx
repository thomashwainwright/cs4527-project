...
import { fetchModuleDetails, fetchModuleAssignments, commitModuleOfferingDetailChanges, commitAssignmentData } from "../api/modules";
...
export default function ModuleDetails() {
  ...
  function updateStateData(row: HTMLTableRowElement, assignment: AssignmentRow) {
    const newDelta = Number(row.cells[1].textContent); // todo validate type
    const newShare = Number(row.cells[2].textContent);

    const changed = newDelta != assignment.delta || newShare != assignment.share // detect if input has resulted in any change, no point changing if not.
  ...
  function markDel(assignment: AssignmentRow, value: boolean) {
    setModuleAssignments(prev =>
      prev?.filter(item => !(item.unique_id === assignment.unique_id && typeof item.new === "number")).map(item =>
        item.unique_id === assignment.unique_id
          ? { ...item, del: value }
          : item
      )
          ...
          {!moduleNotFound ? <div className="flex mt-10 gap-4 flex-col md:flex-row text-2xl">
            ...
            <div className="lg:w-1/2 pr-16 flex flex-col">
              ...
              <div className="mt-4 flex flex-row">
                ...
                <input
                  className="border border-gray-300 rounded-md p-2 ml-auto bg-gray-50"
                  value={moduleDetails.module_type[0].toUpperCase() + moduleDetails.module_type.slice(1).split("_").join("/")}
                  disabled
                />
              </div>
              ...
              {moduleDetails.module_type == "teaching" && <p className="mt-4 flex flex-row mb-8">
              ...
              {moduleDetails.module_type == "teaching" ? <div className="ring-2 rounded-[1px] ring-gray-200 ring-offset-12">
            ...
            {moduleDetails.module_type == "teaching" && <div className="lg:w-1/2 pr-16">
              {" "}
              <div className="flex flex-row">
                <b>Assignments</b>
                <div className="ml-auto flex gap-2">
                  {
                  editAssignments && <>
                      <button className={"border border-gray-200 rounded-md px-4 py-2 cursor-pointer text-gray text-xl text-gray-700 hover:bg-gray-200"} onClick={() => setAssignMode(true)} title="Add">Add</button>
                      <button className={"border border-gray-200 rounded-md px-4 py-2 cursor-pointer text-gray text-xl text-gray-700 hover:bg-gray-200"} onClick={() => saveAssignmentData()} title="Save changes to assignments.">Save</button>
                    </>
                  }
                  <button className={"border border-gray-200 rounded-md px-4 py-2 cursor-pointer text-xl " + (editAssignments ? "bg-blue-600 text-white hover:bg-blue-400" : "text-gray-700 hover:bg-gray-200")} onClick={() => setEditAssignments(!editAssignments)} title="Edit">{editAssignments ? "Stop editing" : "Edit"}</button>
                  {assignMode && <Fullscreen open={assignMode} onClose={() => setAssignMode(false)}><AssignModule offering_id={moduleDetails.offering_id} onAdd={(staff) => addAssignments(staff)} moduleAssignments={moduleAssignments}/></Fullscreen>}
                </div>
              </div>
                    ...
                    moduleAssignments.map((assignment: AssignmentRow) => (
                      assignment.user_id && <tr
                        ...
                        <td className="px-4 py-2 border">
                          ...
                          <select
                            name="coordinator"
                            className={!editAssignments ? "appearance-none" : "w-full"}
                            ...
                            onChange={(e) => {
                              ...
                              setModuleAssignments(prev =>
                                prev?.map(item =>
                                  item.unique_id === assignment.unique_id
                                    ? { ...item, coordinator: Number(value), edit: true }
                                    : item
                                )
                              );
                            }}
                        ...
                        {editAssignments ? <td className="group-hover:bg-white bg-white" contentEditable={false}>
                          <button aria-label="Delete" title="Delete module" className="hover:bg-gray-200 ml-2 rounded-lg" onClick={(e) => {e.stopPropagation(); markDel(assignment, !assignment.del)}}><img alt="" src={deleteIcon} className="w-6 m-2"/></button>
                        </td> : <></>}
                      </tr>
                    ))}
                </tbody>
