...
export function Staff() {
  ...
  useEffect(() => {
    fetchStaff().then((staff: Staff[]) => {
      ...
      fetchAllStaffAssignments(selectedYear.year_id).then((module_data: CombinedAssignmentType[]) => {
        setData(staff.map(s => {
          const allocation = Number(100 * Number(module_data
            .filter(m => m.user_id === s.user_id)
            .reduce(
              (sum: number, m: CombinedAssignmentType) => {
                return sum +
                Number(
                  evaluateFormula(
                    m,
                    m.custom_formula ?? default_formula(m.module_type)
                  )
                )},
              0
    ...
    <div className="flex flex-col h-dvh p-12">
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        <table className="min-w-full text-xl ">
          ...
          <tbody>
            {getFilteredData().map((staff: Staff & {allocation: number}) => (
              staff.active && <tr
                key={staff.user_id}
