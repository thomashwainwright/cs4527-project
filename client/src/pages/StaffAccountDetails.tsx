...
export default function AccountDetails() {
  ...
  useEffect(() => {
    ...
    fetchStaffByEmail(email).then(
      (staff_data: Staff & { pw_changed: boolean }) => {
        setStaff(staff_data);
      },
    );
    ...
    <div className="flex mt-10 gap-4 flex-col md:flex-row text-2xl">
      <div className="lg:w-1/2 pr-16 flex flex-col">
          ...
          <input
            ...
            onChange={(e) => {
              setStaff({
                ...
                contract_hours: ["TS", "TR", "TRP1", "TRP2"].includes(
                  e.target.value,
                )
                  ? defaultHours(e.target.value)
        ...
        <div className="flex mt-4 ml-auto">
          <button
            className={
              "border border-gray-200 rounded-md px-4 py-2 cursor-pointer text-xl text-white bg-red-500 hover:bg-red-400 mr-4"
            }
          ...
          <button
            ...
            title="Save changes to staff member."
          >
            Save
          </button>
          <Fullscreen
